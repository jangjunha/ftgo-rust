...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            ...
            put(update_courier_availability),
        )
...
pub async fn get_delivery_status(
    ...
    let delivery_info = delivery_status
        .delivery_info
        .ok_or(ApiError::ServiceUnavailable(
            "Invalid delivery status".to_string(),
        ))?;
    ...
    Ok(Json(DeliveryStatusResponse {
        ...
        pickup_time: delivery_info
            .pickup_time
            .and_then(|ts| chrono::DateTime::from_timestamp(ts.seconds, ts.nanos as u32)),
        delivery_time: delivery_info
            .delivery_time
            .and_then(|ts| chrono::DateTime::from_timestamp(ts.seconds, ts.nanos as u32)),
        courier_actions: delivery_status
            .courier_actions
            .into_iter()
            .map(|action| match action.r#type {
                0 => "PICKUP".to_string(),
                1 => "DROPOFF".to_string(),
                _ => "UNKNOWN".to_string(),
            })
...
pub async fn get_courier(
    ...
    let request = tonic::Request::new(GetCourierPayload {
        courier_id: courier_id.clone(),
    });

    let response = delivery_client.get_courier(request).await.map_err(|e| {
        if e.code() == tonic::Code::NotFound {
    ...
    let actions = plan
        ...
        .map(|action| {
            Ok::<CourierActionResponse, ApiError>(CourierActionResponse {
                ...
                scheduled_time: {
                    let timestamp = action.time.ok_or(ApiError::ServiceUnavailable(
                        "Invalid action time".to_string(),
                    ))?;
